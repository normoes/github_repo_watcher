...
class Db():
    ...
    def insert_(self, table, args):
        ...
        log.warn('Insert {args} into table {table}'.format(table=table,
                                                           args=args))
        ...
        with self.db_engine.connect() as connection:
            try:
                ...
                # i = insert(table_)
                # statement = i.values(args)
                # connection.execute(i, args)
            except Exception as e:
                log.exception(e)

    def create_db_tables(self):
        self.releases = Table(RELEASES, self.metadata,
        ...
        self.commits = Table(COMMITS, self.metadata,
                             Column('id', Integer, primary_key=True,
                                    autoincrement=True),
                             Column('repo', String),
    ...
    def release_exists(self, release):
        ...
        return self.session_query(exists().where(and_(
